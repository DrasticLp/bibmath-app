;(function ($, window, document, undefined) {
  ...
  Plugin.prototype = {
    getTimeRemaining: function()
    {
	  ...
	  else return this.settings.seconds;
    },
...
/*    extendTimer: function (value) {
    ...
    restart: function () {
	  ...
	  // To prevent to open two setIntervals, we close the previous one if it is not closed.
	  if (this.inteval!=0)
	...
	{
		this.settings.seconds=sec;
	},
	

    stop: function (cb) {
      clearInterval(this.interval);
	  this.secondsmemory+=new Date().getTime() - this.startedAt.getTime();
      if (cb) { cb(); }
    },
    ...
    _draw: function () {
      var millisElapsed;
  //    millisElapsed = new Date().getTime() - this.startedAt.getTime()+this.decalage;
      millisElapsed = new Date().getTime() - this.startedAt.getTime()+this.secondsmemory;
	  
      this.secondsElapsed = Math.floor((millisElapsed)/1000);
      ...
      if (this.secondsElapsed < this.settings.seconds) {
        ...
        this._drawCountdownLabel(this.secondsElapsed);
      } else {

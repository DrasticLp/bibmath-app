...
function affiche_sousmenu(num)
{
 ...
 // On affiche ce qui nous int�resse...  
...
function affiche_sousmenumobile(num)
{
	...
	if (sousmenu.style.display!="block")
	{
		...
		// Le sous menu n'�tait pas affich�, on l'affiche
...
// Ouvre et ferme la fen�tre de recherche mobile
...
// Le spoiler sp�cifique aux exercices
...
function showCorrige(idinner,idimage,idexo)
{
	showAll(idinner,idimage);  // On affiche le contenu ...
	
	
	// On fait notre requête Ajax pour mettre à jour la base de données
	
	var xhrcorrige;
	
	xhrcorrige=getXMLHttpRequest();  // On crée la requête
	
	console.log(idexo),
   	
   	xhrcorrige.open("GET", "lib/affichecorrige.php?id=" +idexo, true);  // Requête asynchrone...
   	
   	// On modifie le texte affiché...
   	

   	
   	xhrcorrige.onreadystatechange = function() 
   	{
		xhrcorrige.abort(); // On met fin à la requête si besoin
	};
   	
   	// On lance la requête.
   	
   	xhrcorrige.send(null);	
}
